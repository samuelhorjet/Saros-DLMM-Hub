...
const PositionsPageContent = () => {
  ...
  const fetchAllUserPositions = useCallback(
    async (forceRefresh: boolean = false) => {
      ...
      try {
        ...
        console.error(
          "A critical error occurred during the fetch process:",
          err
        );
  ...
  const renderContent = () => {
    ...
    if (allEnrichedPositions.length === 0) {
      return (
        <p className="text-center text-muted-foreground py-10">
          {statusMessage || "No positions found."}
        </p>
      );
    }
      ...
      <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
        <div className="animate-slide-up space-y-2">
          ...
          <p className="text-muted-foreground">
            An overview of all your liquidity positions across all pools.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Search by symbol or pool address..."
              className="pl-10"
              value={searchTerm}
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-center">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="desc">
                  Sort by Liquidity: High to Low
                </SelectItem>
                <SelectItem value="asc">
                  Sort by Liquidity: Low to High
                </SelectItem>
              </SelectContent>
            </Select>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
                size="icon"
                onClick={() => fetchAllUserPositions(true)}
                disabled={isLoading}
              >
                <RefreshCw
