...
const CopyButton: React.FC<{ textToCopy: string }> = ({ textToCopy }) => {
  ...
  return (
    <Button variant="ghost" size="icon" onClick={handleCopy} className="h-6 w-6 text-muted-foreground"><Copy className="h-3 w-3" /><span className="sr-only">Copy</span></Button>
  );
};

const InfoRow: React.FC<{ label: string; value?: React.ReactNode; children?: React.ReactNode; isAddress?: boolean }> = ({ label, value, children, isAddress }) => (
    <div className="flex items-center justify-between py-3 border-b">
        <span className="text-sm text-muted-foreground">{label}</span>
        <div className={`flex items-center gap-2 text-sm ${isAddress ? 'font-mono' : 'font-medium'}`}>
            {value}
            {children}
        </div>
    </div>
...
const logoStyle: React.CSSProperties = { width: 40, height: 40, borderRadius: "50%" };
const FallbackLogo: React.FC<{ symbol?: string }> = ({ symbol }) => (<div style={{...logoStyle, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: 'bold' }} className="bg-muted">{symbol ? symbol.charAt(0).toUpperCase() : '?'}</div>);
const PairLogos: React.FC<{ baseToken: TokenInfo; quoteToken: TokenInfo; }> = ({ baseToken, quoteToken }) => (
    <div className="flex items-center">
      ...
      {quoteToken.logoURI ? <img src={quoteToken.logoURI} alt={quoteToken.symbol} style={{...logoStyle, marginLeft: '-12px' }} className="border-2 border-background"/> : <FallbackLogo symbol={quoteToken.symbol} />}
    </div>
);
...
const PositionDetailsContent = () => {
    ...
    if (totalLiquidity === BigInt(0)) { status = 'Empty'; statusVariant = "destructive"; }
    else if (poolDetails.activeId >= position.lowerBinId && poolDetails.activeId <= position.upperBinId) { status = 'Active'; statusVariant = "default"; }
        ...
        <div className="min-h-screen w-full">
            ...
            <main className="p-4 md:p-8 animate-slide-up">
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold">{baseToken.symbol} / {quoteToken.symbol}</h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <Card className="lg:col-span-2">
                       ...
                       <CardContent>
                           <InfoRow label="Status" value={<Badge variant={statusVariant}>{status}</Badge>} />
                    ...
                    <Card>
                        ...
                        <CardContent className="flex flex-col gap-3">
                            <Button onClick={() => setIsRebalanceModalOpen(true)} disabled={isActionDisabled} variant="outline" size="lg">
                                <RefreshCw className="h-4 w-4 mr-2" /> Rebalance
                            </Button>
                            <Button onClick={() => setIsRemoveModalOpen(true)} disabled={isActionDisabled} variant="destructive" size="lg">
                                 <MinusCircle className="h-4 w-4 mr-2" /> Remove Liquidity
                            </Button>
                            {status === 'Empty' && (
                                <Button onClick={() => setIsBurnModalOpen(true)} variant="destructive" size="lg" className="bg-destructive/50 hover:bg-destructive/60">
